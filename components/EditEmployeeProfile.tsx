...
const EditEmployeeProfile: React.FC<EditEmployeeProfileProps> = ({ employeeId, onBack, onViewHistory, isReadOnly = false, userRole }) => {
    ...
    const fetchEmployeeData = async () => {
       ...
       try {
          ...
          if (data) {
             ...
             setBusinessUnit(data.business_unit || 'CollabCRM');
             ...
             try {
                ...
                if (!configError && configData?.config_value) {
                   ...
                   if (configData.config_value.componentOverrides) {
                      setComponentOverrides(configData.config_value.componentOverrides);
                   }
   ...
   const formatDateOfJoining = (dateStr: string) => {
      if (!dateStr) return '';
   ...
   return (
      ...
      <div className="p-4 lg:p-8 w-full space-y-6 animate-in fade-in duration-300">
         ...
         <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col md:flex-row items-center justify-start gap-8 md:gap-12 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="flex items-center gap-4 shrink-0">
               {/* Avatar */}
               <div className="w-14 h-14 rounded-full bg-indigo-500 text-white flex items-center justify-center font-bold text-lg shadow-sm">
               ...
               <div>
                  <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                     {fullName}
                     <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-bold border border-emerald-100 flex items-center gap-1.5">
            ...
            {/* Details Columns */}
            <div className="flex flex-wrap md:flex-nowrap items-center gap-8 md:gap-16 w-full md:w-auto">
               <div className="space-y-1">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Employee Code</p>
                  <p className="text-sm font-bold text-slate-700">{employeeCode}</p>
               </div>
               <div className="space-y-1">
               ...
               <div className="space-y-1">
                  ...
                  <p className="text-sm font-bold text-slate-700">{designation}</p>
               </div>
               <div className="space-y-1">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Business Unit</p>
                  <p className="text-sm font-bold text-slate-700">{businessUnit || 'CollabCRM'}</p>
               </div>
               <div className="space-y-1">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Date of Joining</p>
                  <p className="text-sm font-bold text-slate-700">{formatDateOfJoining(joiningDate)}</p>
               </div>
            </div>
         </div>
                  ...
                  <div className="bg-slate-800 px-6 py-4 flex justify-between items-center text-white">
                     ...
                     {!isReadOnly && (
                        <div className="flex items-center gap-2">
                           ...
                           <div className="flex items-center gap-2">
                               <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Edit Mode</span>
                               <button
                                  onClick={() => setIsSalaryStructureEditable(!isSalaryStructureEditable)}
                                  className={`w-9 h-5 rounded-full p-0.5 transition-all duration-300 flex items-center ${
                                     isSalaryStructureEditable ? 'bg-emerald-500 justify-end' : 'bg-slate-600 justify-start'
                                  }`}
                                  title={isSalaryStructureEditable ? "Disable Editing" : "Enable Editing"}
                               >
