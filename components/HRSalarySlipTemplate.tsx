...
const HRSalarySlipTemplate: React.FC = () => {
  ...
  const handleTagClick = (tag: string) => {
    ...
    let newSuffix = tempSuffix;
    if (newSuffix.includes(tag)) {
        // Remove
        newSuffix = newSuffix.replace(tag, '');
    } else {
        // Add
        // Toggle Month/MonthShort
        if (tag === '{{Month}}' && newSuffix.includes('{{MonthShort}}')) {
             newSuffix = newSuffix.replace('{{MonthShort}}', '{{Month}}');
             setTempSuffix(newSuffix);
             setTempToggles(prev => ({ ...prev, shortMonth: false }));
             return;
        }
        if (tag === '{{MonthShort}}' && newSuffix.includes('{{Month}}')) {
             newSuffix = newSuffix.replace('{{Month}}', '{{MonthShort}}');
             setTempSuffix(newSuffix);
             setTempToggles(prev => ({ ...prev, shortMonth: true }));
             return;
        }

        newSuffix = newSuffix ? `${newSuffix}_${tag}` : tag;
    }

    // Cleanup
    newSuffix = newSuffix.replace(/__+/g, '_').replace(/^_|_$/g, '');
    setTempSuffix(newSuffix);

    // Sync Toggles based on presence
    setTempToggles(prev => ({
        ...prev,
        includeId: newSuffix.includes('{{EmployeeID}}'),
        payPeriod: newSuffix.includes('{{PayPeriod}}'),
        shortMonth: newSuffix.includes('{{MonthShort}}')
    }));
  };

  const generatePreview = (suffix: string) => {
      let preview = 'Payslip_' + suffix;
      preview = preview.replace('{{EmployeeName}}', 'Priya_Sharma');
      preview = preview.replace('{{EmployeeID}}', 'TF00123');
      preview = preview.replace('{{Month}}', 'November');
      preview = preview.replace('{{MonthShort}}', 'Nov');
      preview = preview.replace('{{Year}}', '2025');
      preview = preview.replace('{{CompanyName}}', 'TechFlow');
      preview = preview.replace('{{PayPeriod}}', '01-30_Nov_2025');
      
      // Cleanup multiple underscores just in case
  ...
  const resetToDefault = () => {
      // Default reset is applied to temporary state if editing
      if (isNamingEditing) {
          setTempSuffix('{{Month}}_{{Year}}_{{EmployeeID}}');
          setTempToggles({ includeId: true, shortMonth: false, payPeriod: false });
      }
  };
  ...
  const handleCreate = () => {
      ...
      setTemplateName('');
      setSections({ earnings: [], deductions: [], reimbursements: [], summary: [] });
        ...
        <div className="p-4 lg:p-8 w-full mx-auto space-y-6 animate-in fade-in duration-300">
             <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                 ...
                 <button onClick={handleCreate} className="px-4 py-2 bg-purple-600 text-white text-sm font-bold rounded-lg hover:bg-purple-700 shadow-sm flex items-center gap-2">
                     <Plus size={16} /> Create New Payslip
                 </button>
             </div>
               ...
               <div className="flex items-center gap-4">
                   ...
                   <div>
                       <div className="text-xs text-slate-500 flex gap-2 items-center">
                           <span>Payslip Template</span> <span className="text-slate-300">/</span> <span className="uppercase font-bold text-xs text-purple-600">{view} MODE</span>
                       </div>
                       {isReadOnly ? (
                           ...
                           <input 
                              ...
                              className={`text-lg font-bold text-slate-800 border-b border-transparent hover:border-slate-300 focus:border-purple-500 focus:outline-none bg-transparent px-1 ${validationError && !validationError.includes('Earnings') ? 'border-rose-300 bg-rose-50' : ''}`}
                           />
                       )}
                   </div>
               </div>
               ...
               {activeTab === 'EDITOR' ? (
                   <>
                                ...
                                <div className="p-8 space-y-8">
                                    ...
                                    <div className={`relative border border-slate-100 rounded-xl p-6 transition-all group ${!isReadOnly ? 'hover:border-purple-200 hover:shadow-sm' : ''}`}>
                                        ...
                                        <div className="grid grid-cols-2 gap-y-2 text-sm text-slate-600">
                                            {headerConfig.employeeFields.name && <div>Name: <span className="font-semibold text-slate-800">Priya Sharma</span></div>}
                                            {headerConfig.employeeFields.id && <div>ID: <span className="font-semibold text-slate-800">TF00123</span></div>}
                                            ...
                                            {headerConfig.employeeFields.department && <div>Dept: <span className="font-semibold text-slate-800">Engineering</span></div>}
                                        </div>
                                    </div>
                       ...
                       <div className="w-80 bg-white border-l border-slate-200 p-6 overflow-y-auto">
                            ...
                            <div className="space-y-6">
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-3">Global Configuration</label>
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between">
                                            <span className="text-sm text-slate-700">Currency</span>
                                            <span className="text-xs font-bold bg-slate-100 px-2 py-1 rounded">INR (₹)</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-sm text-slate-700">Date Format</span>
                                            <span className="text-xs font-bold bg-slate-100 px-2 py-1 rounded">DD MMM YYYY</span>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-3">Toggles</label>
                                    <div className="space-y-4">
                   ...
                   <div className="flex-1 bg-slate-100 p-8 flex justify-center overflow-y-auto">
                       <div className="w-[210mm] min-h-[297mm] bg-white shadow-2xl p-12 flex flex-col relative text-slate-900" style={{ fontFamily: 'Times New Roman, serif' }}>
                           ...
                           <div className="grid grid-cols-2 gap-y-3 gap-x-8 mb-8 text-sm border-b border-slate-200 pb-6">
                               ...
                               {headerConfig.employeeFields.location && <div className="flex"><span className="w-32 font-bold text-slate-600">Location</span><span>: Bangalore</span></div>}
                               {headerConfig.employeeFields.pan && <div className="flex"><span className="w-32 font-bold text-slate-600">PAN</span><span>: ABCDE1234F</span></div>}
                               ...
                               {headerConfig.employeeFields.bankAccount && <div className="flex"><span className="w-32 font-bold text-slate-600">Bank A/c</span><span>: HDFC0001234</span></div>}
                           </div>
                           ...
                           <table className="w-full border-collapse border border-slate-300 mb-8 text-sm">
                               <thead>
                                   <tr className="bg-slate-100">
                                       <th className="border border-slate-300 p-2 text-left w-[35%]">Earnings</th>
                                       <th className="border border-slate-300 p-2 text-right w-[15%]">Amount</th>
                                       <th className="border border-slate-300 p-2 text-left w-[35%]">Deductions</th>
                                       <th className="border border-slate-300 p-2 text-right w-[15%]">Amount</th>
                                   </tr>
                               </thead>
                               <tbody>
                                   ...
                                   {Math.max(sections.earnings.length, sections.deductions.length) < 3 && (
                                       <tr><td className="border-l border-r border-slate-300 h-8"></td><td className="border-r border-slate-300"></td><td className="border-r border-slate-300"></td><td className="border-r border-slate-300"></td></tr>
                                   )}
...
export default HRSalarySlipTemplate;
