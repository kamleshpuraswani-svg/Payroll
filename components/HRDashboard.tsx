...
const HRDashboard: React.FC = () => {
  ...
  const [activePayrollModal, setActivePayrollModal] = useState<'ACTIONS' | 'RUN' | 'SCHEDULE' | 'SETTINGS' | null>(null);
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
                ...
                <div className="p-6 grid grid-cols-1 gap-3">
                    ...
                    <button 
                      onClick={() => setActivePayrollModal('SETTINGS')}
                      className="flex items-center gap-4 p-4 border border-slate-200 rounded-xl hover:bg-purple-50 hover:border-purple-200 transition-all group text-left"
                    >
                        <div className="w-12 h-12 bg-purple-100 text-purple-600 rounded-full flex items-center justify-center shrink-0 group-hover:bg-purple-200 transition-colors">
                            <Settings size={24} />
                        </div>
                        <div>
                            <h4 className="font-bold text-slate-800 group-hover:text-purple-800">Payroll Settings</h4>
                            <p className="text-xs text-slate-500 mt-0.5">Configure payslips and automation</p>
                        </div>
                        <ChevronRight className="ml-auto text-slate-300 group-hover:text-purple-400" size={18} />
                    </button>
                </div>
            </div>
        </div>
