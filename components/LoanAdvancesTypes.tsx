...
const MOCK_LOAN_TYPES: LoanType[] = [
  { id: '1', name: 'Personal Loan', interestRate: 10.5, maxAmount: '5,00,000', maxTenure: 24, status: true, description: 'Standard personal loan for employees.' },
  { id: '2', name: 'Festival Advance', interestRate: 0, maxAmount: '20,000', maxTenure: 6, status: true, description: 'Special advance for festival expenses.' },
  { id: '3', name: 'Salary Advance', interestRate: 0, maxAmount: '2 months gross salary', maxTenure: 6, status: true, description: 'Advance against upcoming salary.' },
...
const LoanAdvancesTypes: React.FC = () => {
  const [loanTypes, setLoanTypes] = useState<LoanType[]>(MOCK_LOAN_TYPES);
  ...
  const activeTypesCount = loanTypes.filter(t => t.status).length;
  const totalActiveLoans = 142; // Static as per requirement
  ...
  const handleAddNew = () => {
    setCurrentLoan({
      ...
      interestRate: 0,
      maxAmount: '',
      maxTenure: 0, // Default 0 as requested
      status: true,
      description: ''
    });
  ...
  const handleSave = () => {
    ...
    if (!currentLoan.name || !currentLoan.name.trim()) {
        setErrors({ name: 'Loan Name is required.' });
        return;
    }

    if (currentLoan.id) {
      // Update
      setLoanTypes(prev => prev.map(t => t.id === currentLoan.id ? { ...t, ...currentLoan } as LoanType : t));
    } else {
      // Add
      const newType: LoanType = {
        ...
        maxAmount: currentLoan.maxAmount || '0',
                    ...
                    <div className="p-8 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="md:col-span-2">
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Loan Name <span className="text-rose-500">*</span></label>
                                <input 
                                    type="text" 
                                    value={currentLoan.name} 
                                    onChange={e => {
                                        setCurrentLoan({...currentLoan, name: e.target.value});
                                        if (e.target.value.trim()) setErrors({});
                                    }} 
                                    placeholder="e.g. Personal Loan" 
                                    className={`w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 ${errors.name ? 'border-rose-500 bg-rose-50' : 'border-slate-200'}`} 
                                />
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    onChange={e => setCurrentLoan({...currentLoan, interestRate: parseFloat(e.target.value) || 0})} 
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    onChange={e => setCurrentLoan({...currentLoan, maxTenure: parseInt(e.target.value) || 0})} 
                                    className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500" 
                                />
                            </div>
