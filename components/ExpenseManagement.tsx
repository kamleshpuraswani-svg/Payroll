...
const ExpenseManagement: React.FC = () => {
  ...
  // Summary Stats
  const stats = [
    { label: 'Total Claims This Month', val: '184', color: 'text-slate-800' },
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] bg-slate-50 animate-in fade-in duration-300 relative">
      ...
      {/* Success Toast */}
      {showSuccessToast && (
          <div className={`absolute top-6 left-1/2 -translate-x-1/2 z-[110] px-6 py-3 rounded-full shadow-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4 ${
              showSuccessToast.type === 'success' ? 'bg-emerald-600 text-white' : 'bg-rose-600 text-white'
          }`}>
