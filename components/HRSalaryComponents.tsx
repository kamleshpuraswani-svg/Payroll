...
const EARNINGS_FIELDS = [
    { name: 'Component Name', icon: Tag },
    { name: 'Nature of Pay', icon: FileText },
    { name: 'Calculation Method', icon: Calculator },
    { name: 'Taxable', icon: CheckCircle },
];

const DEDUCTIONS_FIELDS = [
    { name: 'Component Name', icon: Tag },
    { name: 'Nature of Deduction', icon: FileText },
    { name: 'Taxable', icon: CheckCircle },
];
...
const HRSalaryComponents: React.FC = () => {
    ...
    const scFields = activeTab === 'Earnings' ? EARNINGS_FIELDS : DEDUCTIONS_FIELDS;

    const getOperatorsForField = (field: string) => {
        if (field === 'Component Name') return ['Is', 'Contains'];
        return ['Is'];
    };

    const getOptionsForField = (field: string) => {
        if (field === 'Nature of Pay') return ['Fixed Pay', 'Variable Pay'];
        if (field === 'Nature of Deduction') return ['Variable'];
        if (field === 'Taxable') return ['Yes', 'No'];
        const uniqueValues = new Set<string>();
        components.forEach(c => {
            if (c.category !== activeTab) return;
            if (field === 'Component Name') uniqueValues.add(c.name);
            if (field === 'Calculation Method' && c.calculationMethod) uniqueValues.add(c.calculationMethod);
        });
    ...
    const scSelectField = (field: string) => { setCurrentField(field); setCurrentOperator(null); setTempValues([]); setTempContainsText(''); setValSearchQuery(''); };
    const scSelectOperator = (op: string) => { setCurrentOperator(op); setTempValues([]); setTempContainsText(''); setValSearchQuery(''); };
    const scToggleTempValue = (val: string) => { setTempValues(prev => prev.includes(val) ? prev.filter(v => v !== val) : [...prev, val]); };
    ...
    const scApplyFilter = () => {
        if (currentField && currentOperator) {
            ...
            if (vals.length > 0 && (currentOperator !== 'Contains' || vals[0].trim() !== '')) {
                setCompletedFilters(prev => [...prev, { id: Math.random().toString(), field: currentField, operator: currentOperator, values: vals }]);
                setCurrentField(null); setCurrentOperator(null); setTempValues([]); setTempContainsText(''); setValSearchQuery(''); setDropdownOpen(false);
            }
        }
    };

    const scCancelFilter = () => { setCurrentField(null); setCurrentOperator(null); setTempValues([]); setTempContainsText(''); setValSearchQuery(''); setDropdownOpen(false); };
    const scRemoveFilter = (id: string) => { setCompletedFilters(prev => prev.filter(f => f.id !== id)); };
    const scClearAllFilters = () => { setCompletedFilters([]); scCancelFilter(); setSearchQuery(''); };
    ...
    const filteredData = useMemo(() => {
        ...
        for (const filter of completedFilters) {
            result = result.filter(c => {
                let val = '';
                if (filter.field === 'Component Name') val = c.name || '';
                else if (filter.field === 'Nature of Pay' || filter.field === 'Nature of Deduction') val = c.natureOfPay || c.category || '';
                else if (filter.field === 'Calculation Method') val = c.calculationMethod || '';
                else if (filter.field === 'Taxable') val = c.isTaxable ? 'Yes' : 'No';
        ...
        return result;
    }, [components, activeTab, selectedTarget, searchQuery, completedFilters]);
                        ...
                        <div className="flex items-center gap-2 w-full sm:flex-1">
                            <div className="relative flex-1" ref={dropdownRef}>
                                <div
                                    ...
                                    className="w-full flex flex-wrap items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm min-h-[40px] focus-within:ring-2 focus-within:ring-purple-500/20 focus-within:border-purple-500 transition-all cursor-text pr-10 shadow-sm"
                                >
                                    {completedFilters.length === 0 && !currentField && (
                                        <Search className="h-4 w-4 text-slate-400 shrink-0" />
                                    )}
                                    {completedFilters.map(filter => {
                                        const fObj = scFields.find(f => f.name === filter.field);
                                        ...
                                        return (
                                            <div key={filter.id} className="flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-lg px-2.5 py-0.5 text-xs font-semibold text-slate-700">
                                                ...
                                                <button onClick={(e) => { e.stopPropagation(); scRemoveFilter(filter.id); }} className="ml-1 text-slate-400 hover:text-slate-600 transition-colors"><X size={12} /></button>
                                            </div>
                                        );
                                    })}
                                    {currentField && (
                                        <div className="flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-lg px-2 py-0.5 text-xs font-bold text-slate-700">
                                            {(() => { const fObj = scFields.find(f => f.name === currentField); const FIcon = fObj?.icon; return FIcon ? <FIcon size={12} className="text-slate-500" /> : null; })()}
                                    ...
                                    <input
                                        ref={inputRef}
                                        type="text"
                                        value={currentField && currentOperator ? valSearchQuery : searchQuery}
                                        onChange={(e) => {
                                            if (currentField && currentOperator) { setValSearchQuery(e.target.value); }
                                            else { setSearchQuery(e.target.value); setDropdownOpen(false); }
                                        }}
                                    ...
                                    />
                                    {(completedFilters.length > 0 || currentField || searchQuery) && (
                                        <button onClick={(e) => { e.stopPropagation(); scClearAllFilters(); }} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-all">
                                            <X size={14} />
                                        </button>
                                    )}
                                </div>
                                    ...
                                    <div className="absolute top-full left-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-slate-200 z-50 py-1.5 animate-in fade-in slide-in-from-top-1 duration-150">
                                        {!currentField && (
                                            <div className="py-1">
                                                ...
                                                {scFields.map(f => (
                                                    <button key={f.name} onClick={() => scSelectField(f.name)} className="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2.5 transition-colors">
                                        ...
                                        {currentField && !currentOperator && (
                                            <div className="py-1">
                                                ...
                                                {getOperatorsForField(currentField).map(op => (
                                                    <button key={op} onClick={() => scSelectOperator(op)} className="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2.5 transition-colors">
                                        ...
                                        {currentField && currentOperator && (
                                            <div className="flex flex-col max-h-[300px]">
                                                            ...
                                                            {(() => {
                                                                ...
                                                                return opts.map(opt => (
                                                                    <label key={opt} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-slate-700 hover:bg-slate-50 cursor-pointer transition-colors">
                                                                        <input type="checkbox" checked={tempValues.includes(opt)} onChange={() => scToggleTempValue(opt)} className="rounded text-purple-600 focus:ring-purple-500 cursor-pointer w-3.5 h-3.5" />
                                                                        <span>{opt}</span>
                                                                    </label>
                                                                ));
                                                            })()}
                                                ...
                                                <div className="p-2 bg-slate-50 border-t border-slate-100 flex justify-end gap-2">
                                                    <button onClick={scCancelFilter} className="px-2.5 py-1 text-[10px] text-slate-500 font-bold hover:text-slate-700 transition-colors">Cancel</button>
                                                    <button onClick={scApplyFilter} disabled={currentOperator === 'Contains' ? tempContainsText.trim() === '' : tempValues.length === 0} className="px-3 py-1 bg-purple-600 text-white text-[10px] font-bold rounded hover:bg-purple-700 transition-colors disabled:opacity-50">Done</button>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                            ...
                            <button onClick={() => setDropdownOpen(!dropdownOpen)} className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-700 hover:bg-slate-50 text-sm font-medium flex items-center gap-2 shadow-sm shrink-0 h-[40px]">
