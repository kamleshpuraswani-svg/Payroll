...
const DocCard = ({ title, description, status, date, onPreview, hidePreview, downloadParts, monthDropdown }: { title: string, description: string, status: 'ready' | 'upcoming' | 'na' | 'link', date?: string, onPreview?: () => void, hidePreview?: boolean, downloadParts?: string[], monthDropdown?: boolean }) => {
  ...
  const handleDownload = (partLabel?: string) => {
    const docTitle = partLabel ? `${title} - ${partLabel}` : title;
    const element = document.createElement("a");
    const file = new Blob([`Content for ${docTitle}`], {type: 'text/plain'});
    element.href = URL.createObjectURL(file);
    element.download = `${docTitle.replace(/\s+/g, '_')}.txt`;
  ...
  const isLocked = status === 'upcoming' && !downloadParts;
          ...
          {monthDropdown ? (
             <>
               <div className="relative flex-1">
                  <select
                     ...
                     className="w-full h-11 pl-4 pr-8 border border-blue-500 rounded-lg text-sm font-bold text-slate-700 bg-white appearance-none focus:outline-none focus:border-blue-500 cursor-pointer"
                  >
                     {FY_MONTHS.map(m => (
               ...
               <button
                  ...
                  className="flex-1 h-11 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-100"
               >
             ...
             <>
               ...
               <button
                  ...
                  className={`h-11 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-100 ${hidePreview ? 'w-full text-sm' : 'w-11'}`}
               >
                  <Download size={18}/> {hidePreview && 'Download'}
               </button>
             </>
          ) : isLocked ? (
