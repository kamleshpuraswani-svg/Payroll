    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="flex flex-col items-end gap-1">
          {stat.detailsAtTop && stat.extraDetails && stat.extraDetails.length > 0 && (
            <div className="text-right mb-2 space-y-1">
              {stat.extraDetails.map((detail, idx) => (
                <div key={idx} className="flex justify-end items-center gap-4 text-[10px]">
                  ...
                  <span className={`font-black ${detail.color ? detail.color : 'text-slate-700'}`}>{detail.value}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div>
        ...
        <div className="flex items-baseline gap-2">
          ...
          <span className={`flex items-center text-xs font-semibold ${stat.trendUp ? 'text-emerald-600' : stat.trend === 'Due within 3 days' ? 'text-rose-600' : 'text-slate-500'}`}>
            ...
            {!stat.trendUp && stat.trend && stat.trend.includes('%') && <ArrowDownRight size={14} />}
            {stat.trend}
          </span>
        </div>
      </div>
