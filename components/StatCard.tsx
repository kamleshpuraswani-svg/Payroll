...
const StatCard: React.FC<StatCardProps> = ({ stat }) => {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="flex flex-col items-end gap-1">
          {stat?.details_at_top && stat?.extra_details && stat.extra_details.length > 0 && (
            <div className="text-right mb-2 space-y-1">
              {stat.extra_details.map((detail, idx) => (
                <div key={idx} className="flex justify-end items-center gap-4 text-[10px]">
                  <span className="text-slate-400 font-bold uppercase tracking-wider">{detail?.label || 'N/A'}</span>
                  <span className={`font-black ${detail?.color ? detail.color : 'text-slate-700'}`}>{detail?.value || 'N/A'}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="flex items-center gap-1">
            ...
            {stat?.on_info_click && (
              <button
                onClick={(e) => { e.stopPropagation(); stat.on_info_click?.(); }}
      ...
      <div>
        <h3 className="text-slate-500 text-sm font-medium mb-1">{stat?.title || 'N/A'}</h3>
        <div className="flex items-baseline gap-2">
          <span className="text-2xl font-bold text-slate-800">{stat?.value || 'N/A'}</span>
          <span className={`flex items-center text-xs font-semibold ${stat?.trend_up ? 'text-emerald-600' : stat?.trend === 'Due within 3 days' ? 'text-rose-600' : 'text-slate-500'}`}>
            {stat?.trend_up && <ArrowUpRight size={14} />}
            {!stat?.trend_up && stat?.trend && stat.trend.includes('%') && <ArrowDownRight size={14} />}
            {stat?.trend || ''}
          </span>
        </div>
      </div>
        ...
        <div className="mt-6 pt-4 border-t border-slate-100 space-y-2.5">
          {stat.extra_details.map((detail, idx) => (
            <div key={idx} className="flex justify-between items-center text-xs">
              ...
              <span className={`font-bold ${detail?.color ? detail.color : 'text-slate-700'}`}>{detail?.value || 'N/A'}</span>
            </div>
          ))}
        </div>
      )}
